...
public abstract class SectionedBaseAdapter extends BaseAdapter implements PinnedHeaderListView.PinnedSectionedHeaderAdapter {
   ...
   */
  private SparseIntArray mSectionPositionCache;
  /**
   ...
   */
  private SparseIntArray mSectionCountCache;
  ...
  public SectionedBaseAdapter() {
    super();
    mSectionCache = new SparseIntArray();
    mSectionPositionCache = new SparseIntArray();
    mSectionCountCache = new SparseIntArray();
    mCount = -1;
    mSectionCount = -1;
  }
  ...
  public final int getSectionForPosition(int position) {
    ...
    int cachedSection = mSectionCache.get(position);
    if (cachedSection != 0) {
  ...
  public int getPositionInSectionForPosition(int position) {
    // first try to retrieve values from cache
    int cachedPosition = mSectionPositionCache.get(position);
    if (cachedPosition != 0) {
  ...
  private int internalGetCountForSection(int section) {
    int cachedSectionCount = mSectionCountCache.get(section);
